...
description: "Duplicate of Claude /verify-*. Use when the user invokes /verify-*, $verify-output-contract, or asks to run this RLVerify command. Use the standard RLVerify result card contract with OUTCOME, EVIDENCE, WEIGHT, DETAIL, NEXT, closed outcome vocabularies, evidence weights, and phase-specific tables."
---

# /verify-* — Codex Duplicate

This is a Codex skill duplicate of `.claude/commands/verify-output-contract.md`. The Claude slash command name is `/verify-*`; in Codex, invoke it as `$verify-output-contract` or write `/verify-*` in the prompt. Interpret Claude-specific Task/Bash references as workflow instructions and use Codex-native tools or the local RLVerify CLI.
