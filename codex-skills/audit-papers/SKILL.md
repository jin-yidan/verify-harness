---
name: audit-papers
description: "Duplicate of Claude /audit-papers. Use when the user invokes /audit-papers, $audit-papers, or asks to run this RLVerify command. Search arXiv for RL theory papers, fetch them, run RLVerify paper verification on lemmas and theorems, and maintain a structured audit log."
---

# /audit-papers — Codex Duplicate

This is a Codex skill duplicate of `.claude/commands/audit-papers.md`. The Claude slash command name is `/audit-papers`; in Codex, invoke it as `$audit-papers` or write `/audit-papers` in the prompt. Interpret Claude-specific Task/Bash references as workflow instructions and use Codex-native tools or the local RLVerify CLI.
