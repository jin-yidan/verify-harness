---
name: verify-library
description: "Duplicate of Claude /verify-library. Use when the user invokes /verify-library, $verify-library, or asks to run this RLVerify command. Standalone library growth — evaluate a verified novel lemma for addition to RLGeneralization, generalize it to its atomic reusable form, re-run the redundancy and back-translation gates, then add_novel"
---

# /verify-library — Codex Duplicate

This is a Codex skill duplicate of `.claude/commands/verify-library.md`. The Claude slash command name is `/verify-library`; in Codex, invoke it as `$verify-library` or write `/verify-library` in the prompt. Interpret Claude-specific Task/Bash references as workflow instructions and use Codex-native tools or the local RLVerify CLI.
