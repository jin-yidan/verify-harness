---
name: verify-rl-paper
description: "Duplicate of Claude /verifyRL-paper. Use when the user invokes /verifyRL-paper, $verify-rl-paper, or asks to run this RLVerify command. Verify a multi-lemma paper proof with RLVerify by extracting dependency structure, detecting circularity, applying the full verification process to each component, and producing a paper report."
---

# /verifyRL-paper — Codex Duplicate

This is a Codex skill duplicate of `.claude/commands/verifyRL-paper.md`. The Claude slash command name is `/verifyRL-paper`; in Codex, invoke it as `$verify-rl-paper` or write `/verifyRL-paper` in the prompt. Interpret Claude-specific Task/Bash references as workflow instructions and use Codex-native tools or the local RLVerify CLI.
...
This skill is a wrapper around the RLVerify BYO-agent harness. Do **not** hand-run
`python -m rlverify.ingest`, instantiate `VerifyDriver`, or manually drive
`PaperSession` from this skill. Those are internal implementation details owned
by the harness path.
...
```bash
python3 -m harness verify <paper-or-fixture> --backend codex --report
```
...
```bash
python3 -m harness verify <paper-or-url> --theorem "<label>" --backend codex --dry-run
python3 -m harness verify <paper-or-url> --all-theorems --backend codex --dry-run
```
...
```bash
python3 -m harness verify <paper-or-url> --theorem "<label>" --backend codex --report
python3 -m harness verify <paper-or-url> --all-theorems --backend codex --report
```
...
```bash
python3 -m harness verify <paper-or-url> --backend codex --dry-run
```
...
1. Run `python3 -m harness verify ... --dry-run` with `--backend codex`.
...
4. Rerun without `--dry-run`; let `harness.runner` launch the authenticated Codex driving agent and sealed
   grader calls.
...
```bash
python3 -m harness verify "<arg>" --theorem "<label>" --backend codex --dry-run
```
