---
name: verify-resolve
description: "Duplicate of Claude /verify-resolve. Use when the user invokes /verify-resolve, $verify-resolve, or asks to run this RLVerify command. Standalone resolve phase — decompose a proof into atomic blocks and classify each as library match / instantiation / novel by searching the corpus and running type-directed library_search"
---

# /verify-resolve — Codex Duplicate

This is a Codex skill duplicate of `.claude/commands/verify-resolve.md`. The Claude slash command name is `/verify-resolve`; in Codex, invoke it as `$verify-resolve` or write `/verify-resolve` in the prompt. Interpret Claude-specific Task/Bash references as workflow instructions and use Codex-native tools or the local RLVerify CLI.
